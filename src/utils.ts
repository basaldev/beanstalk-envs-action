...
import { Entry, ClassifiedEntry, ResolvedEntry, AWSConfig, AWSClientConfig, SecretEntry, DirectEntry, SecretGroups } from './types';
...
export function extractEntries(
  ...
  try {
    const secretsEntries: ClassifiedEntry[] = Object.entries(JSON.parse(awsSecretReferencesInput)).map(([key, value]) => ({
      key,
      value: value as string | boolean | number,
      type: 'aws_secret_reference'
    }));
    
    const directEntries: ClassifiedEntry[] = Object.entries(JSON.parse(jsonInput)).map(([key, value]) => ({
      key,
      value: value as string | boolean | number,
      type: 'direct_value'
    }));
 ...
 */
export function sortEntries<T extends Entry | ClassifiedEntry>(array: T[]): T[] {
...
export async function resolvePartialReferencesToValues(
  ...
  for (let secretIndex = 0; secretIndex < secretNames.length; secretIndex++) {
    ...
    try {
      core.debug(`Resolving secret ${secretName} for ${secretEntries.length} keys...`);
      ...
      for (let entryIndex = 0; entryIndex < secretEntries.length; entryIndex++) {
        ...
        if (value !== undefined) {
          ...
          core.debug(`Resolved ${entry.key}: ${entry.originalValue} -> [RESOLVED]`);
        } else {
          // Secret key not found in the secret
          core.debug(`Secret key ${entry.secretKey} not found in secret ${secretName} for entry ${entry.key}`);
          resolvedEntries.push({
            key: entry.key,
      ...
      core.debug(`Failed to resolve secret ${secretName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
 ...
 */
async function fetchSecretData(client: SecretsManagerClient, secretName: string): Promise<Record<string, any>> {
