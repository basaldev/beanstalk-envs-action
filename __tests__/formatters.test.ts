...
describe('ebextensionsEnvVarsSecretManagerFormatter', () => {
  ...
  it('should format AWS secret references with correct namespace', () => {
    const result = ebextensionsEnvVarsSecretManagerFormatter(mockEntries, 'ap-northeast-1');
    
    expect(result).toContain('namespace: aws:elasticbeanstalk:application:environmentsecrets');
    expect(result).toContain('option_name: APP_ENVIRONMENT');
    expect(result).toContain('arn:aws:secretsmanager:ap-northeast-1:${AWS::AccountId}:secret:projectname-dev-shared-vars:APP_ENVIRONMENT');
  });

  it('should format direct values with correct namespace', () => {
    const result = ebextensionsEnvVarsSecretManagerFormatter(mockEntries, 'ap-northeast-1');
    
    expect(result).toContain('namespace: aws:elasticbeanstalk:application:environment');
  ...
  it('should generate valid YAML structure', () => {
    ...
    expect(result).toMatch(/  - namespace: aws:elasticbeanstalk:application:environmentsecrets\n    option_name: APP_ENVIRONMENT\n    value: arn:aws:secretsmanager:ap-northeast-1:\$\{AWS::AccountId\}:secret:projectname-dev-shared-vars:APP_ENVIRONMENT/);
    expect(result).toMatch(/  - namespace: aws:elasticbeanstalk:application:environment\n    option_name: NODE_ENV\n    value: development/);
  });

  it('should handle different AWS regions', () => {
    const result = ebextensionsEnvVarsSecretManagerFormatter(mockEntries, 'eu-west-1');
    
    expect(result).toContain('arn:aws:secretsmanager:eu-west-1:${AWS::AccountId}:secret:projectname-dev-shared-vars:APP_ENVIRONMENT');
  });
});
